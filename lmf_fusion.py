...
class LMF_fusion(nn.Module):
    ...
    def forward(self, mRNA_x, meth_x, miRNA_x, W=False):
        ...
        if W:
            ...
            print(f"mRNA modality importance: {mRNA_mean}")
            print(f"Methylation modality importance: {meth_mean}")
            print(f"miRNA modality importance: {miRNA_mean}")
            ...
            ax1.set_title('Comparison of Modality Importance', fontsize=16, fontweight='bold')
            ax1.set_ylabel('Average L2 Norm Value', fontsize=12)
            ...
            ax2.set_title('Modality Importance Distribution', fontsize=16, fontweight='bold')

            plt.tight_layout()
            plt.savefig("modality_importance_bar.png", dpi=300, bbox_inches='tight')
            plt.close(fig)
