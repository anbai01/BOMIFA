...
def load_trained_model(in_dim, hgcn_dim, i, num, data_folder, dropout=0.4):
    combined_path = f'./{data_folder}/models/{num}/View{i}_combined.pth'
    combined_state = torch.load(combined_path)

    gcn_e_model = GCN_E(in_dim, hgcn_dim, dropout)
    gcn_e_model.load_state_dict(combined_state['E'])
    gcn_e_model.eval()

    trans_f_model = all(hgcn_dim[-1], 5, 1600, 10, 0.3)
    trans_f_model.load_state_dict(combined_state['H'])
    trans_f_model.eval()

    gcn_pare_model = ContrastiveTrainer(hgcn_dim[-1], 0.1)
    gcn_pare_model.load_state_dict(combined_state['P'])
    gcn_pare_model.eval()

    classifier_model = Classifier(hgcn_dim[-1], 1)
    classifier_model.load_state_dict(combined_state['C'])
